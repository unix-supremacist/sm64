...
u32 save_file_get_star_flags(s32 fileIndex, s32 courseIndex) {
    u32 starFlags;
    starFlags = gSaveBuffer.files[fileIndex].courseStars[courseIndex+1] & STAR_MASK;
    return starFlags;
}
...
void save_file_set_star_flags(s32 fileIndex, s32 courseIndex, u32 starFlags) {
    gSaveBuffer.files[fileIndex].courseStars[courseIndex+1] |= starFlags;
...
s32 save_file_is_cannon_unlocked(void) {
    return (gSaveBuffer.files[gCurrSaveFileNum - 1].courseStars[gCurrCourseNum] & (1 << STAR_COUNT)) != 0;
}
...
void save_file_set_cannon_unlocked(void) {
    gSaveBuffer.files[gCurrSaveFileNum - 1].courseStars[gCurrCourseNum] |= (1 << STAR_COUNT);
    gSaveBuffer.files[gCurrSaveFileNum - 1].flags |= SAVE_FLAG_FILE_EXISTS;
    gSaveFileModified = TRUE;
}
